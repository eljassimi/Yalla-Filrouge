        ...
        <div class="flex flex-col md:flex-row items-center justify-between mb-8">
            <div class="flex items-center justify-between w-full">
                ...
                <div class="flex flex-col items-center">
                    ...
                    <div class="flex flex-col items-center">
                        <span class="text-white font-bold text-base mt-1">PAYMENT</span>
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($transports as $transport)
                <div class="bg-darkgray shadow-xl rounded-lg overflow-hidden transform transition-transform hover:-translate-y-2">
                    <div class="h-56 bg-lightgray relative">
                        <img src="/{{$transport["logo"]}}" alt="{{$transport["name"]}} Logo" class="w-full h-full object-contain p-4">
                    </div>
                    <div class="p-6">
                        <div class="flex justify-between items-start mb-3">
                            <h3 class="text-xl text-white font-bold">{{$transport["name"]}}</h3>
                        </div>
                        <p class="text-sm mb-4 text-gray-300">{{ substr($transport['description'], 0, 100) . '...' }}</p>
                        <div class="flex justify-between items-center mt-4">
                            <div>
                                <span class="font-bold text-red-600 bg-red-500 bg-opacity-20 px-4 py-1 rounded-xl">Bus</span>
                            </div>
                            <a href="/transport-details/{{$transport["id"]}}" class="bg-primary hover:bg-primary/90 text-white px-4 py-2 text-sm font-medium rounded-md transition">
                                Details
                            </a>
                        </div>
                    </div>
